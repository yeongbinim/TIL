...
scanner.next()의 인자로 정규표현식을 넘길 수 있는데, number는 솔직히 nextInt() 굳이 안쓰고 이렇게 하는 이유가 있나? 싶을 것이다.

1. 0과 양의 정수만을 입력 받아야 한다.
2. 공통적으로 next()를 사용하는 함수로 묶기 위해서이다.

함수로 묶는 부분(= 프로시저 추상화)을 살펴보자.
...
공통된 구현 메서드를 공유하지 않는 지금의 상황에서 인터페이스가 더 목적에 맞다고 판단했기 때문이다.
...
```
그리고 Calculator 클래스가 calculate를 실행하는 시점에서 Operation 인터페이스를 통해 주입된 인스턴스를 실행하도록 구현했다.

인터페이스에 의존하게끔 해서 DIP 원칙을 지킨 것이다.
...
어떤 입력 타입을 넣었는지 모르니, 어떤 반환 타입이 나올지 모른다는 점. 따라서 이때 제네릭을 사용해야 한다.
